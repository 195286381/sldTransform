...
function genSLDFile(str, outputDir) {
    ...
    const rootObj = genGridSLDRootJSONObj(propertyName, divisionNum, divisionAry, colorAry, divisionDescriptionAry, [], sldTitle);
    
    // 生成对应XML.
    fs.writeFile(path.join(outputDir, filename), xml(rootObj, {declaration: true,indent: true}), function(err) {
        if (err) throw(err);
    });
}
